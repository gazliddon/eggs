...
(defn draw-font-stuff [font shader]

  (font/start-text font shader {:u_radii (vec2 0.1 0.1)
                                :u_proj mat/M44
                                :u_hardness (vec2 0.1 0.1)
                                :u_outer_color (vec4 1 1 1 0.7)
                                :u_inner_color (vec4 1 1 1 0.7)
                                :u_model(scale (vec3 1 -1 1))
                                :u_view (scale (vec3 0.1 0.1 1)) })
...
(defn update! [gl t shader]
  ...
  (let [{:keys [aspect]} (glw/update-wh! gl-window)
    ...
    (draw-font-stuff font-printer shader))
